...
import { getCardActive,getPromotion,getService,updatePayment } from '../../../api/API';
...
const PayDetails = () => {
  ...
  const selectNumber = (number,cardType,idCard,money,fullname,phoneNumber,idUser) => {
    ...
    >
      <ScrollView contentContainerStyle={{ flexGrow: 1,height:hp('100%')}}>
                ...
                <View style={styles.containerContent}>
                  <View>
                      <TouchableOpacity style={styles.inputWrapper} onPress={toggleServiceOptions}>
                          {/* Icon của chọn số */}
                                ...
                                {Object.keys(listService).map((key,index) => {
                                      ...
                                      if (list.useYn) {
                                        return (
                                          ...
                                          onPress={() => selectService(list.serviceName,list.duration,list.totalAmount,list.idPricing,list.idService)}
                                        >
                                          <Text style={styles.optionText}>{list.serviceName} -- Time:{list.duration}p</Text>
                                        </TouchableOpacity>
                                        );
                                      }
                                      return null;
                                ...
                                {Object.keys(numbers).map((key) => {
                                      ...
                                      if (num.useYn) {
                                        return (
                                          ...
                                          key={num.idCard}
                                          style={styles.dropdownItem}
                                          onPress={() => selectNumber(num.cardNo,num.cardType,num.idCard,num.availableBalance,num.fullName,num.phoneNumber,num.idUser)}
                                        >
