      ...
      <ScrollView style={[styles.scrollContainer]}>
        <View style={[styles.containerSearch]}>
          ...
          <View style={styles.cardTypeContainer}>
            ...
            >
              ...
              {cardType !== "0" && (
                <Icon name="circle-o" style={styles.iconCard} color="#724929" />
              )}
            </TouchableOpacity>
            <TouchableOpacity
              style={styles.cardTypeButton}
        ...
        <View style={[styles.history]}>
          ...
          {Object.keys(listTransac).map((key) => {
            ...
            if (ts.useYn) {
              const itemStyle =
                index % 2 === 0 ? styles.evenItem : styles.oddItem; // Áp dụng style màu nền so le
              index++; // Tăng biến đếm khi giao dịch được hiển thị
              return (
                <TouchableOpacity
                  style={styles.button}
                  onPress={() =>
                    navigation.navigate("HistoryTransac", { transacInfo: ts })
                  }
                  key={ts.idTransaction}
                >
